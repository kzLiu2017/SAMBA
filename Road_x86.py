...
def Matrix_decrease(block_matrix, block_matrix_t, block_information):
    ...
    for i in range(len(block_matrix)):
        if  (sum(block_matrix[i]) == 1)& (sum(block_matrix_t[i]) >= 1) :
...
for nnn in range(len(Func_name)):
    ...
    for i in range(len(block_address)):
        ...
        while address <= block_address[i][1]:
            strin1 = idc.GetOpnd(address, 0)
            strin1 = '.' + strin1[1:]
            if strin1 in Func_name:
                print("33333333333333333---------", strin1)
                block_information[i].append(strin1)
                store_or_not = 1

            if idc.GetOpnd(address, 0) in Func_name:
                print("33333333333333333---------", idc.GetOpnd(address, 0))
...
def DFS_road_read(matrix, matrix_t, Test_target, Final_roads, SSL_judge, inline, down_to_up_list):
    global add
    ...
    if entrance == -1:  # ALL SSL
        ...
        while road_count >= 0:  # for each road
            ...
            # go on DFS
            DFS_road_read(matrix, matrix_t, Test_target_new, Final_roads, SSL_judge, inline, down_to_up_list)
    ...
    for i in range(len(Func_name)):  # for all func
        if down_to_up_list[i] == 0:  # for not ready
            ...
            if all_road_sum == len(All_Road[i]):
                ...
                DFS_road_read(matrix, matrix_t, Test_target, Final_roads, DFS_SSL, 3, down_to_up_list)
                add = 0
...
All_Road[c] = []
for k in range(len(Final_roads)):
    if ("SSL_connect" in Final_roads[k]) | (".SSL_connect" in Final_roads[k]):  # fliter not connect
...
for ii in range(len(father_fun_name)):
    ...
    for i in range((func.endEA - func.startEA) / 4):
        ...
        if (idc.GetMnem(address) == "BL") & (idc.GetOpnd(address, 0) == "SSL_CTX_set_verify"):
            ...
            parement[address] = value_parement[1:]
print(parement)
