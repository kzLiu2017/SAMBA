...
time_file = open(result_path + folder + "/time_result.txt" , "a+")
...
SSL_IO_function = ['SSL_write', 'SSL_read', '.SSL_write', '.SSL_read', '_SSL_write', '_SSL_read']
...
#if ('SSL_CTX_set_verify' not in function_mark) & (('SSL_get_peer_certificate' not in function_mark) | ('SSL_get_verify_result'not in function_mark)) & (('SSL_CTX_set_options' not in function_mark) & ('SSL_set_options' not in function_mark) & ('SSL_CTX_set_min_proto_version' not in function_mark) & ('SSL_CTX_max_proto_version' not in function_mark)) :
if int_bool == False:
    output = open(result_path + folder + '/Report_method.txt', 'a')
    output.write(machine)
    for version_API in SSL_version_function:
        if version_API in function_mark:
            output.write(version_API)
            output.write('\n')
    output.close()
    output = open(result_path + folder + '/Report_verify.txt', 'a')
    output.write('Roads do not use SSL_CTX_set_verify')
    output.write('\n')
    output.write('Roads do not use SSL_get_peer_certificate | SSL_get_verify_result')
    output.close()
    ...
    elif "main" in function_Mark:
        s_index = function_Mark.index('main')
    elif "start" in function_Mark:
        s_index = function_Mark.index('start')
    ...
    if 'main' in function_Mark:
        ...
        for i in range(len(matrix[m_index])):
            if (matrix[m_index][i] == 1) & (function_Mark[i] != '_start') & (function_Mark[i] != "start"):
                matrix[m_index][i]=0
    ...
    for i in range(len(function_Mark)):
        if sum(matrix[i]) == 0:
            if (function_Mark[i] != "_start") and (function_Mark[i] != "main") and (function_Mark[i] != "_ftext") and (function_Mark[i] != "start"):
                del_list.append(i)
                j_i = j_i + 1
    ...
    def Dfs_visit(matrix, node, visit, father):
        n = len(matrix)
        visit[node] = 1
        for i in range(n):  # for all nodes
            if matrix[i][node] == 1:  # search son in order
                if visit[i] == 1:  # &(i!=father[node])# son is grey && not point to father
                    ...
                    Dfs_visit(matrix, i, visit, father)
        visit[node] = 2
    ...
    for j in range(len(function_Mark)):
        if sum(matrix[j]) == 0:
            if visit[j] == 0:
                Dfs_visit(matrix, j, visit, father)
    print('\n***************************************del leave')
    count_which_del = []
    for i in range(n):
    ...
    time_file.write("callgraph " + str(run_time) + " ")
    print('\n**************************************save file')
    with open(result_path + folder + "/fun_name.txt", "w") as f:
        for i in range(len(function_Mark)):
    ...
    # have circle or not
    def Dfs_visit_block(node, visit, father, block_address, cycle_in_cfg):
        ...
        for i in range(n):  # for all nodes
            if block_matrix_t[i][node] == 1:  # search son in order
                if visit[i] == 1:  # &(i!=father[node])# son is grey && not point to father
                    ...
                    father[i] = node  # father[]: grey node
                    Dfs_visit_block(i, visit, father, block_address, cycle_in_cfg)
        visit[node] = 2
    ...
    def Matrix_decrease_pre(block_matrix, block_matrix_t, block_information):
        count_which_del = [] # ascending order list # store which function needed to be deleted
        for i in range(len(block_matrix)):
            if (block_information[i][0] == "None") & (sum(block_matrix[i]) == 1) & (sum(block_matrix_t[i]) == 1):
                count_which_del.append(i)
    ...
    def delete_block_first(block_matrix, block_matrix_t, block_information):
        del_block_list = []
        tmp_block_matrix = copy.deepcopy(block_matrix)
        tmp_block_matrix_t = copy.deepcopy(block_matrix_t)
        for i in range(len(block_matrix)):
            if (sum(block_matrix_t[i]) == 1) and (sum(block_matrix[i]) == 1) and (block_information[i] == ['None']):
                pre_node_index = 0
                after_node_index = 0
                del_block_list.append(i)
                for j in range(len(block_matrix)):
                    if block_matrix[i][j] == 1:
                        after_node_index = j
                    elif block_matrix_t[i][j] == 1:
                        pre_node_index = j
                tmp_block_matrix[pre_node_index][after_node_index] = 1
                tmp_block_matrix_t[after_node_index][pre_node_index] = 1
        counter = 0
        for index in del_block_list:
            del tmp_block_matrix[index - counter]
            del tmp_block_matrix_t[index - counter]
            del block_information[index - counter]
            for j in range(len(tmp_block_matrix_t)):
                del tmp_block_matrix_t[j][index - counter]
                del tmp_block_matrix[j][index - counter]
            counter = counter + 1
        block_matrix = []
        block_matrix = copy.deepcopy(tmp_block_matrix)
        block_matrix_t = []
        block_matrix_t = copy.deepcopy(tmp_block_matrix_t)
        return block_matrix, block_matrix_t
    ...
    for nnn in range(len(Func_name)):
        ...
        for i in range(len(block_address)):
                    ...
                    if x_name == None:
                        ...
                        for xref_sub in DataRefsFrom(address):
                            ...
                            if ((idc.print_insn_mnem(address) != "lea") & (idc.print_insn_mnem(address) != "mov")) & (machine == "X86-64"):
                                ...
                                continue
                            if get_func_name(xref_sub) != None:
                                x_name = get_func_name(xref_sub)
                                if (x_name in Func_name) & (x_name != name):
                                    break_xref_loop = True
                            else:
                                for xref_sub_sub in DataRefsFrom(xref_sub):
                                    x_name = get_func_name(xref_sub_sub)
                                    if (x_name in Func_name) & (x_name != name):
                                        break_xref_loop = True
                                        break
                            if break_xref_loop == True:
                                break
            ...
            if store_or_not == 0:
                ...
                if block_address[i][0] == func.start_ea:
                    ...
                    block_information[i].append("func_end")
        ...
        print(len(block_matrix))
        block_matrix, block_matrix_t = delete_block_first(block_matrix, block_matrix_t, block_information)
        print(len(block_matrix))
        #Matrix_decrease_pre(block_matrix, block_matrix_t, block_information)
        ...
        cycle_in_cfg = []
        Dfs_visit_block(0, visit, father, block_address, cycle_in_cfg)
        ...
        for i in range(len(new_block)):
            tmp_block_information = []
            for j in range(len(block_information[new_block[i]])):
                if "func_start" in block_information[new_block[i]][j]:
                    tmp_block_information.append(block_information[new_block[i]][j])
                elif "func_end" in block_information[new_block[i]][j]:
                    tmp_block_information.append(block_information[new_block[i]][j])
                elif "None" in block_information[new_block[i]][j]:
                    tmp_block_information.append(block_information[new_block[i]][j])
                elif "0x" not in block_information[new_block[i]][j]:
                    if matrix[Func_name.index(block_information[new_block[i]][j])][nnn] == 1:
                        tmp_block_information.append(block_information[new_block[i]][j])
                        tmp_block_information.append(block_information[new_block[i]][j + 1])
                elif "0x" in block_information[new_block[i]][j]:
                    continue
            if len(tmp_block_information) == 0:
                tmp_block_information.append("None")
            new_block_information.append(tmp_block_information)
        ...
        for f_road_index in range(len(func_road)):
            ...
            while "func_end" in func_road[f_road_index]:
                func_road[f_road_index].remove("func_end")
        for f_road_index in range(len(func_road)):
        ...
        for road in range(len(Func_road)):
            if "exit" in Func_road[road]:
                exit_index = Func_road[road].index("exit")
                Func_road[road].remove("exit")
                del Func_road[road][exit_index]
        ...
        road_key_list = []
        del_index_list = []
        for road_index in range(len(Func_road)):
            road_key = []
            for road_node in Func_road[road_index]:
                if "0x" not in road_node:
                    road_key.append(road_node)
            if road_key not in road_key_list:
                road_key_list.append(road_key)
            else:
                del_index_list.append(road_index)
        counter = 0
        for index in del_index_list:
            del Func_road[index - counter]
            counter =  counter + 1
    ...
    end_time = time.perf_counter()
    run_time = end_time - start_time
    time_file.write("intra-cfg " + str(run_time) + " ")
    ...
    def DFS_road_read2(Test_target, Final_roads, SSL_judge, inline):
        global add
        # find first not SSL func
        entrance = -1
        if len(Test_target)>0:
            if Test_target[0] in filtered_API:
                return
        for a in range(len(Test_target)):
            if ('0x' not in Test_target[a]) & (Test_target[a] not in SSL_judge):
                entrance = a
        ...
        if entrance == -1:  # ALL SSL
            ...
            while road_count >= 0:  # for each road
                ...
                for q in range(len(Test_target)):
                    Test_target_new.append(Test_target[q])
                del Test_target_new[entrance]
                #Test_target_new[entrance] = '$_' + Test_target_new[entrance] + '_[Midroad_' + str(road_count + 1) + ']'
                # update target_new
                for j in range(len(All_Road[func_index][road_count])):
                    h = len(All_Road[func_index][road_count]) - j - 1
                    Test_target_new.insert(entrance, All_Road[func_index][road_count][h])
                # go on DFS
                DFS_road_read2(Test_target_new, Final_roads, SSL_judge, inline)
                road_count = road_count - 1
                        ...
                        if matrix_new[i][j] == 1: # function j call function i
                            ...
                            for m in range(len(All_Road_new[j])): #single road of parent
                                ...
                                if Func_name[i] in All_Road_new[j][m]:
                                    ...
                                    for n in All_Road_new[j][m]: # every function in single road
                                        if n == Func_name[i]:  # function in road that need to be replaced
                            ...
                            for single_road_tmp in single_road_list_all:
                                if single_road_tmp not in single_road_list_tmp:
                                    ...
                                    if (last_non_io_func > first_io_func) & (first_io_func != -1) & (last_non_io_func != -1) & (machine != "X86-64"):
                                        remove_road = True
    ...
    #         if ("0x" not in road_ele) and (road_ele in SSL_function):
    ...
    for k in range(len(Final_roads)):
        ...
        if version_function > 1:
            continue
        if ("SSL_connect" in Final_roads[k]) | ("BIO_connect" in Final_roads[k]) | ("SSL_do_handshake" in Final_roads[k]): # fliter not connect
            IO_function = False
                    ...
                    if (("SSL_read" not in Final_roads[k][0]) & ("SSL_write" not in Final_roads[k][0]) & (Final_roads[k][0] != "SSL_get_peer_certificate") & ("0x" not in Final_roads[k][0]) & ("BIO_write" not in Final_roads[k][0]) & ("BIO_read" not in Final_roads[k][0])):
                        ...
                        if "SSL_connect" in Final_roads[k]:
                            ...
                            if "SSL_read" in Final_roads[k]:
                                ...
                                #     continue
                        # if "SSL_CTX_set_verify" in Final_roads[k]:
                        #     SSL_CTX_set_verify_index = Final_roads[k].index("SSL_CTX_set_verify")
                        #     if SSL_new_index > SSL_CTX_set_verify_index:
                        #         continue
    ...
    end_time = time.perf_counter()
    run_time = end_time - start_time
    time_file.write("extraction " + str(run_time) + " ")
    ...
    def parament_analysis(target_fun):
                ...
                while judge == 0:
                    ...
                    if ('SSL_CTX_set_verify' in target_fun)|('SSL_set_verify' in target_fun):
                        if idc.print_operand(addr, 0) == 'esi':
                            new_addr = addr
                            while new_addr > idc.get_name_ea_simple(father_fun_name[ii]):
                                new_addr = new_addr - 1
                                if (idc.print_operand(new_addr, 0) == "edx") and (idc.print_insn_mnem(addr) == "xor") and (idc.print_operand(new_addr, 1) == "edx"):
                                    print("no call_back")
                                    break
                            if idc.print_insn_mnem(addr) == "xor":
                                ...
                                parement_value_mark = 1
                    elif ('SSL_CTX_set_options' in target_fun):
                        if idc.print_operand(addr, 0) == 'esi':
                            value_parement = idc.print_operand(addr, 1)
                            if value_parement == "20000h":
                                ...
                                parement[str(hex(call_addr[ii]))] = value_parement
                                parement_value_mark = 1
                            elif "2000000h" in value_parement:
                                judge = 1
                                parement[str(hex(call_addr[ii]))] = value_parement
                                parement_value_mark = 1
                    elif ("SSL_CTX_ctrl" in target_fun):
                        if idc.print_operand(addr, 0) == 'esi':
                            if (idc.print_operand(addr - 5, 0) == 'edx') and (idc.print_operand(addr, 1) == '7Bh'):
                                value_parement = idc.print_operand(addr - 5, 1)
                                print("value of ctrl", value_parement, hex(addr))
                    ...
                    if ('SSL_CTX_set_verify' in target_fun)|('SSL_set_verify' in target_fun):
                        if idc.print_operand(addr, 0) == 'R1':
                            if idc.print_insn_mnem(addr) == "xor":
                                ...
                                value_parement = idc.print_operand(addr, 1)
                                if value_parement == "#0x7B":
                                    judge = 1
                                    parement[str(hex(call_addr[ii]))] = value_parement[1:]
                                    parement_value_mark = 1
                                elif value_parement == "#0":
                                    print("addr:",hex(addr), "value:", value_parement[1:])
                                    print("incorrect use SSL_CTX_set_verify")
                                    judge = 1
                                    parement[str(hex(call_addr[ii]))] = value_parement[1:]
                                    parement_value_mark = 1
                    elif ('SSL_CTX_set_options' in target_fun):
                        print(hex(addr), idc.print_operand(addr, 0), idc.print_operand(addr, 1), "2000000" in idc.print_operand(addr, 1))
                        if idc.print_operand(addr, 0) == 'R1':
                            value_parement = idc.print_operand(addr, 1)
                            if value_parement == "20000":
                                judge = 1
                                parement[str(hex(call_addr[ii]))] = value_parement
                                parement_value_mark = 1
                            elif "2000000" in value_parement:
                                judge = 1
                                parement[str(hex(call_addr[ii]))] = value_parement
                                parement_value_mark = 1
                    elif ("SSL_CTX_ctrl" in target_fun):
                        if idc.print_operand(addr, 0) == 'R1':
                            if (idc.print_operand(addr - 5, 0) == 'R2') and (idc.print_operand(addr, 1) == '7Bh'):
                                value_parement = idc.print_operand(addr - 5, 1)
                                print("value of ctrl", value_parement, hex(addr))
        else:#mips
            for ii in range(len(father_fun_name)):
                ...
                while judge == 0:
                    ...
                    if ('SSL_CTX_set_verify' in target_fun)|('SSL_set_verify' in target_fun):
                        ...
                        if idc.print_operand(addr, 0) == '$a1':
                            if (idc.print_operand(addr - 5, 0) == '$a2') and (idc.print_operand(addr, 1) == '7Bh'):
                                value_parement = idc.print_operand(addr - 5, 1)
                                print("value of ctrl", value_parement, hex(addr))
        end_time = time.perf_counter()
        run_time = end_time - start_time
        ...
        if line.find("parameter")>=0:
            time_line = line[line.find("parameter"):]
            time_line = time_line[time_line.find(" ") + 1:]
            patime = time_line[:time_line.find(" ")]
            run_time = float(patime) + run_time
            line = line[:line.find("parameter")]
        time_file.truncate(0) 
        time_file.write(line + "parameter " + str(run_time) + " ")
    target_fun='None'
    start_time_com = time.perf_counter()
    print(start_time_com)
    ...
    for i in range(len(All_Road[c])):
        ...
        if ('TLS_client_method' in All_Road[c][i])|('TLS_method' in All_Road[c][i]):
            ...
            if 'SSL_CTX_ctrl'in All_Road[c][i]:
                ...
                print("123", All_Road[c][i][n], parement, All_Road[c][i][n])
                if All_Road[c][i][n] in parement:
                    if parement[All_Road[c][i][n]] == '0x7B':
                        pm_value = parameter_analysis_min_version(All_Road[c][i][n])[1:]
                        if (pm_value == "0x302") | (pm_value == '') | (pm_value == "0x303"):#set_min SSL3.0
                            Method[i] = 6.5
                        else:
                            Method[i] = 6 # higher than ssl3.0
            if ('SSL_set_options' in All_Road[c][i]) | ('SSL_CTX_set_options' in All_Road[c][i]) | ('.SSL_set_options' in All_Road[c][i]) | ('.SSL_CTX_set_options' in All_Road[c][i]):
                ...
                if All_Road[c][i][n] in parement:
                    print(parement[All_Road[c][i][n]],'2000000' in parement[All_Road[c][i][n]])
                    if '2000000' in parement[All_Road[c][i][n]]:
                        Method[i] = 6.6
        ...
        if ('SSL_CTX_set_verify' not in All_Road[c][i])&('SSL_set_verify' not in All_Road[c][i]):
            ...
            if parement_value_mark==1:
                ...
                print("parement[All_Road[c][i][n]]=='0'", parement[All_Road[c][i][n]], parement[All_Road[c][i][n]]=='0', 'SSL_get_peer_certificate' not in All_Road[c][i], 'SSL_get_verify_result' not in All_Road[c][i])
    ...
    output.write(str(Method.count(6.5)) + ' Roads use TLS_(client)_method and SSL_CTX_set_min_proto_version and the minimum version is higher than TLS 1.2')
    output.write('\n')
    ...
    output.close()
    end_time = time.perf_counter()
    run_time = end_time - start_time_com
    time_file.write("comparation " + str(run_time) + " ")
    print(end_time)
    ...
    with open(result_path + folder + "/time.txt", "w") as f:
